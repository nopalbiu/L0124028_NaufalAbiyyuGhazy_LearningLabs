(function($) {
	...
	$main._show = function(id, initial) {
		...
		if (locked || (typeof initial != 'undefined' && initial === true)) {
			...
			$header.hide(); $footer.hide(); $main.show(); $article.show().addClass('active');
			locked = false;
	...
	if (location.hash != '' && location.hash != '#') $window.on('load', function() { $main._show(location.hash.substr(1), true); });
})(jQuery);
